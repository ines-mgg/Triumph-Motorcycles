...
describe('RepairHistory', () => {
  ...
  describe('getBreakdownsByMotorcycle', () => {
    it("devrait retourner les breakdowns correspondant à l'ID de la motocyclette", () => {
      ...
      expect(repairHistory.getBreakdownsByMotorcycle(motorcycleId)).toContain(
        breakdown1,
      );
      expect(
        repairHistory.getBreakdownsByMotorcycle(motorcycleId),
      ).not.toContain(breakdown2);
    });
