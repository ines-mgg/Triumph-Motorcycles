...
import { Driver, DrivingRecord } from '../driver';
import {
  InvalidLicenseError,
  ExperienceError,
  ContactInfoError,
...
describe('Conducteur', () => {
  ...
  beforeEach(() => {
    conducteur = new Driver('conducteur1', 'Jean Dupont', '123456789', 'A', 5, {
      email: 'jeandurool@example.com',
      phone: '123-456-7890',
    });
  });

  describe('addDrivingRecord', () => {
    it('should add a new driving record into the driving history', () => {
      const record = new DrivingRecord(
        new Date('2023-01-01'),
        'moto1',
        'Test Drive',
        'Essai de routine',
    ...
    it('should send a error if the recording the not valid', () => {
      expect(() => {
  ...
  describe('getDrivingHistory', () => {
    it("should return the driver's history", () => {
      const record = new DrivingRecord(
        new Date('2023-01-01'),
        'moto1',
        'Test Drive',
  ...
  describe('historiqueIncidents', () => {
    it("devrait retourner vrai si l'historique de conduite contient un incident", () => {
      const incident = new DrivingRecord(
        new Date('2023-01-01'),
        'moto1',
        'Incident',
        'Collision mineure',
  ...
  describe('Validation des erreurs de conducteur', () => {
    it('devrait lever une erreur pour un numéro de permis invalide', () => {
      expect(() => {
        new Driver('conducteur2', 'Alice', 'INVALID_LICENSE', 'A', 5, {
    ...
    it("devrait lever une erreur pour des années d'expérience négatives", () => {
      expect(() => {
        new Driver('conducteur3', 'Bob', 'LICENSE123', 'A', -1, {
