...
export class OrderItemDeliveredQuantity {
  ...
  static from(
    deliveredQty: number,
    quantityOrdered: OrderItemQuantityOrdered,
  ): OrderItemDeliveredQuantity | Error {
    if (deliveredQty < 0)
      return new Error('Delivered quantity cannot be negative');
    if (deliveredQty > quantityOrdered.value)
